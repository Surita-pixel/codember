...

{
    ...
    foreach (var kvp in apariciones)
    {
        totales.Add(string.Format($"{kvp.Key}{kvp.Value}"));
    }
    Console.WriteLine(string.Join(string.Empty,totales));
    // Console.Write(string.Join(',', mensaje.Split(' ')));
}

resolverEnigma(mensaje);